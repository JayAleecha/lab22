...
int Unit::ultimateAttack(Unit &c){
    return c.beAttacked(2*atk);
}

Equipment::Equipment(int Ihpmax,int Iatk,int Idef){
    hpmax = Ihpmax;
    atk = Iatk;
    def = Idef;
}
...
void Unit::equip(Equipment *N){
    if(equipment!=NULL){
        vector<int> M = equipment->getStat();
        hpmax -= M[0];
        if(hp>hpmax){
            hp = hpmax;
        }
